...
Function's purpose: Clearing all fields in the <form>
Input: None
Output: None

"validateForm()"

Function's purpose: Verifying that the user's input is expected
Input: None
Output: None
*/
...
We will be using the "document.forms" collection to loop through our form components.
Since we have a name on our form, we are using the form's name as our index in the brackets. 

When we have an error, we will be adding the Bootstrap "has-error" class to the related "form-group" class.
We will use the "parentElement" object to look at the parent of the <input> control that we are validating.
When we check the parent element, we are looking at its class attribute,
specifically checking to see if it contains the string "has-". The "indexOf()" function
indicates the position of where the text appears, and if the text is not found, "indexOf()" returns the value of -1.
...
In this function, we will use the "clearErrors()" function we just created. We will also be calling a function called "focus()",
which allows us to put the cursor in a control and make it the active control. In this case,
it creates a good user experience in that once the user clears the form, their cursor will return to the "num1" textbox.
*/

/*
Unlike "innerText", though, "innerHTML" lets you work with HTML rich text and doesn't automatically encode and decode text.
In other words, "innerText" retrieves and sets the content of the tag as plain text,
...
In validating a form, you are checking to make sure that the user input expected values. In this case,
we want to make sure that the following occurs:
...
We will then use value to get the entered value from the <input> boxes.
If those values are either empty ("") or not a number (isNaN()), then we will:
